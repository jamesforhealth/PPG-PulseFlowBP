...
if __name__=='__main__':
    ...
    if os.path.exists("bp_estimator_conv_lstm.pth"):
        model.load_state_dict(torch.load("bp_estimator_conv_lstm.pth"))
        print("Model loaded from bp_estimator_conv_lstm.pth")
    #參數量
    total_params= sum(p.numel() for p in model.parameters())
    input(f"Total parameters: {total_params}")

    optimizer= optim.Adam(model.parameters(), lr=1e-4, weight_decay=1e-4)
    model= train_model(model, dataloaders, optimizer, num_epochs=1000, device=device)
